...
router.post('/onLogin', (req, res) => {
    ...
    const sql1 = 'SELECT COUNT(*) AS result FROM portfolio WHERE user_id = ?'
    db.query(sql1, user_id, (err, data) => {
        if(!err) {
            ...
            if(data[0].result < 1) {
                ...
                const sql2 = `SELECT 
                                CASE (SELECT COUNT(*) FROM portfolio WHERE user_id = ? AND user_pw = ?)
                                    WHEN '0' THEN NULL
                                    ELSE (SELECT user_id FROM portfolio WHERE user_id = ? AND user_pw = ?)
                                END AS userId
                                , CASE (SELECT COUNT(*) FROM portfolio WHERE user_id = ? AND user_pw = ?)
                                    WHEN '0' THEN NULL
                                    ELSE (SELECT user_pw FROM portfolio WHERE user_id = ? AND user_pw = ?)
                                END AS userPw`;
